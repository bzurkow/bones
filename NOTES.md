...
- [x] ~~Better Auth wired up (Google)~~ — `apps/backend/src/auth.ts` (Drizzle adapter, Google social provider, `trustedOrigins` for the dev client ports). Mounted on Fastify at `/api/auth/*` via a manual Fetch Request/Response bridge, since Better Auth's handler is Fetch-native like Auth.js's. Schema regenerated via `yarn db:auth:generate` (the `@better-auth/cli`) — its generated `users`/`sessions`/`accounts`/`verifications` tables replaced the earlier placeholder `users` table entirely, since Better Auth needs to own that shape; `src/db/schema.ts` now just re-exports `auth-schema.ts` (generated, not hand-edited) plus future domain tables. Verified end to end: `POST /api/auth/sign-in/social` returns a real `accounts.google.com` URL containing our actual `client_id`, the exact redirect URI configured in Google Cloud Console, PKCE challenge, and CSRF state. Google OAuth client credentials live in `apps/backend/.env` (gitignored). **NEXT UP:** an actual sign-in button/flow in `packages/react-ui` + a session-aware tRPC procedure — deliberately not built yet, since the full desktop flow (custom `platypus://` protocol callback per the Auth requirements section, not a raw localhost redirect) is more scope than "wire up the backend."

Two real bugs hit and fixed while wiring this up, worth knowing about if touched again:
