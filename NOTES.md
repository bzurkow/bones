...
- [x] ~~Drizzle schema + `db.ping`~~ — minimal `users` table, first migration generated and applied against local Postgres, `db.ping` (`SELECT 1`) verified end to end through the real tRPC/Fastify/Drizzle/Postgres pipeline. `DbHealthCheck` added to `packages/react-ui` as its own component (not merged into the existing `HealthCheck`), both rendered dev-only on the home screen. **NEXT UP:** Better Auth wired up (Google) — needs a Google OAuth client created in Google Cloud Console first (your account, not something automatable here).
